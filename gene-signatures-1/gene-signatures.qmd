...
- The **slope** indicates the what is the change in the response variable (vec2) given a 1 unit change in the predictor variable (vec1). - The T statistic is the result of a the T test. The T test assesses the significance of individual coefficients in our model and the T value indicates the number of standard errors the estimated coefficient is away from the null hypothesis (t = 0). Remember the T value is the coefficient / standard error.
